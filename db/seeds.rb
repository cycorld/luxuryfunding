...
Dir.glob('./db/md_voca/*.csv').each_with_index do |csv, i|
  ...
  CSV.foreach(csv, {headers: true}) do |r|
    Card.create(chapter_id: chap.id, question: r["question"], answer: r["answer"], a_lang: "en")
  end
end
...
Dir.glob('./db/chin_chinese/*.csv').each_with_index do |csv, i|
  ...
  CSV.foreach(csv, {headers: true}) do |r|
    Card.create(chapter_id: chap.id, question: r["question"], answer: r["answer"], a_lang: "zh")
  end
end
